...
-- Fix devices INSERT policy: only admins can insert
DROP POLICY "Authenticated users can insert devices" ON public.devices;
CREATE POLICY "Admins can insert devices"
  ON public.devices FOR INSERT
  TO authenticated
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

-- Fix devices UPDATE policy: only admins can update
